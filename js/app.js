...
function getWeightedWeather(availableWeatherKeys) {
	const mode = document.getElementById("weatherMode")
		? document.getElementById("weatherMode").value
		: "favored";
	const profile = weatherProfiles[mode];
	const currentPool = availableWeatherKeys.map((w) => ({
		value: w,
		weight: profile[w] || 10,
	}));
...
function parseTracks(data) {
	return data.map((entry) => {
		...
		return {
			...
			category,
			location,
			direction: fullDirection.includes("Left")
				? "Left"
				: fullDirection.includes("Stretch")
					? "Stretch"
					: "Right",
			fullDirection,
...
function getCheckedValues(id) {
	return Array.from(document.querySelectorAll(`#${id} input:checked`)).map(
		(cb) => cb.value,
	);
}
...
function startRoll() {
	...
	function step() {
		...
		if (counter < maxIter) {
			counter++;
			if (counter > 15) speed += 20;
			if (counter > 20) speed += 40;
			setTimeout(step, speed);
		} else {
			finalize(pool[Math.floor(Math.random() * pool.length)]);
		}
	}
	setTimeout(step, 300);
}
...
function triggerManualSearch() {
	...
	const track = allTracks.find(
		(t) => t.name.toLowerCase() === searchVal.toLowerCase(),
	);
	...
	if (track) {
		...
		alert(
			"Track not found! Check your spelling or select from the dropdown list.",
		);
	}
}
...
function generateConditions() {
	...
	const availableWeatherKeys =
		season === "Winter" ? nonSnowyKeys.concat(snowyKeys) : nonSnowyKeys;
...
function finalize(finalTrack) {
	...
	img.addEventListener("load", () => imgTarget.classList.remove("loading"));
	img.addEventListener("error", () => imgTarget.classList.remove("loading"));
	imgTarget.replaceChildren(img);
	imgTarget.classList.add("loading");
...
function handleEasterEgg(key) {
	...
	if (keyBuffer.endsWith("NATURE")) {
		unlockTheme("nature", SECRET_THEMES_INFO.nature);
	}
	if (keyBuffer.endsWith("NEURO")) {
		playEasterSound("sounds/heart.mp3");
		keyBuffer = "";
	}
...
function saveStateToURL() {
	...
	const setParam = (id, key) => {
		const vals = getCheckedValues(id);
		if (
			vals.length > 0 &&
			vals.length < document.querySelectorAll(`#${id} input`).length
		) {
			params.set(key, vals.join(","));
		}
	};
	setParam("terrainFilter", "t");
	setParam("catFilter", "c");
	setParam("dirFilter", "d");
	setParam("capFilter", "p");

	const theme = document.getElementById("themeSelect")?.value;
	if (theme && theme !== "default") params.set("m", theme);

	const weather = document.getElementById("weatherMode")?.value;
	if (weather && weather !== "favored") params.set("w", weather);
...
function loadStateFromURL() {
	...
	try {
		...
		setChecks("terrainFilter", "t");
		setChecks("catFilter", "c");
		setChecks("dirFilter", "d");
		setChecks("capFilter", "p");

		const theme = params.get("m");
		if (theme) {
		...
		const weather = params.get("w");
		if (weather) {
			const el = document.getElementById("weatherMode");
			if (el) el.value = weather;
		}
	} catch (e) {
